<script lang="ts">
	...
	function setTab(tab: Tab) {
		...
		direction =
			tabs.findIndex((t) => t.id === tab) > tabs.findIndex((t) => t.id === activeTab)
				? 1
				: -1;
		...
		>
			{#each tabs as tab (tab.id)}
				<button
					...
					class="relative -mb-px cursor-pointer bg-transparent px-[1.1rem] py-[0.65rem] text-xs font-medium transition-colors duration-150 {activeTab ===
					tab.id
						? 'text-[#7c3aed]'
				...
				>
					...
					{#if activeTab === tab.id}
						<motion.div
							...
							class="absolute inset-x-0 -bottom-[2px] h-[2px] bg-[#7c3aed]"
			...
			<div bind:this={measureEl}>
				<AnimatePresence mode="popLayout" initial={false}>
					{#key activeTab}
						...
						>
							<div
								class="border-b border-[var(--sl-color-hairline)] bg-[var(--sl-color-bg)] px-4 py-3"
							>
								<p
								...
								<p class="mt-1 text-sm text-[var(--sl-color-text)]">
									{activePurpose}
								</p>
							</div>
								...
								>
									...
									<label class="pg-toggle">
										<input
											id="lockCountry-0"
											type="checkbox"
											bind:checked={options.lockCountry}
										/>
										<span class="pg-toggle-knob"></span>
									...
									>
										...
										<select
											...
											onchange={(event) => {
												initialFormat = (
													event.currentTarget as HTMLSelectElement
												).value as 'international' | 'national';
											}}
										>
								...
								>
									<div class="flex flex-wrap items-center gap-2">
										...
										<button
											...
											class="pg-pill {options.allowedCountries?.length
												? ''
												: 'pg-pill-active'}"
										...
										<button
											...
											class="pg-pill {allowedCountriesLabel === 'US, HU'
												? 'pg-pill-active'
												: ''}"
										...
										<button
											class="pg-pill {allowedCountriesLabel === 'US'
												? 'pg-pill-active'
												: ''}"
										...
										<button
											class="pg-pill {allowedCountriesLabel === 'GB, HU, US'
												? 'pg-pill-active'
												: ''}"
							...
							{#if activeTab === 'usage'}
								<div class="p-5">
									<div
										class="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(18rem,0.8fr)]"
									>
										<div
										...
										>
											...
											<h3
												class="mt-2 text-xl font-semibold text-[var(--sl-color-text)]"
											>
												Stay in the loop
											</h3>
											<p
												class="mt-2 text-sm leading-6 text-[var(--sl-color-gray-3)]"
											>
												Add your phone number for delivery updates and
												account recovery.
											</p>
											...
											<div class="mt-5 flex flex-col gap-3">
												...
												<span
													...
													style="display:none">{country ?? ''}</span
												>
												<span
													data-testid="valid-display"
													style="display:none">{String(valid)}</span
												>
												<span
													data-testid="validation-error-display"
													style="display:none"
										...
										>
											...
											>
												...
												>
													Country picker and phone input work as one
													field.
												</li>
												<li
													class="rounded-[0.8rem] border border-[var(--sl-color-hairline)] bg-[var(--sl-color-bg-sidebar)] px-3 py-3"
												...
												<li
													class="rounded-[0.8rem] border border-[var(--sl-color-hairline)] bg-[var(--sl-color-bg-sidebar)] px-3 py-3"
												>
													Saved values stay in E.164, ready for backend
													storage.
												</li>
											</ul>
										</div>
							...
							{:else}
								<div
									class="grid grid-cols-1 {activeTab === 'events'
										? ''
										: 'md:grid-cols-2'}"
								>
									<div
									...
									{#if activeTab === 'events'}
										<div
											...
											class="col-span-full px-5 pb-5 pt-0"
										>
											<div
											...
											>
												...
												>
													<div>
														...
														<p
															class="mt-1 text-sm text-[var(--sl-color-gray-3)]"
														>
															Newest event first.
														</p>
													</div>
												...
												<div class="max-h-[28rem] overflow-y-auto p-3">
													{#if eventLog.length === 0}
														...
														>
															Type into the input to see callbacks
															appear here.
														</p>
													{:else}
											...
											>
												...
												>
													...
													>
														...
														{:else}
															<span
																class="text-[var(--sl-color-gray-4)]"
																>—</span
															>
														{/if}
														<span
												...
												{#if activeTab === 'api' && checkResult !== null}
													...
													>
														...
														>
															...
															<span
																data-testid="check-validity-result"
																style="display:none"
